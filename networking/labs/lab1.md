...
When everything is ready, list your default Docker networks by running sudo docker network ls.

You'll see bridge, host, and none.

You can now create a new network: run sudo docker network create --driver bridge net1

This creates a net1 network. You could run sudo docker network create --help if you wanted to get a quick look at some help documentation, showing you what command arguments are available.

Confirm the network now exists and view its configuration by running sudo docker network inspect net1.
...
sudo docker exec -it <CONTAINER ID> shMake sure you use the correct CONTAINER ID (you can use Control+Shift+V to paste), the one you copied in the previous task.
